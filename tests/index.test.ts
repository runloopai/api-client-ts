...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['RUNLOOP_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Runloop({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['RUNLOOP_BASE_URL'] = 'https://example.com/from_env';
    ...
    test('in request options overridden by client options', () => {
      const client = new Runloop({ bearerToken: 'My Bearer Token', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['RUNLOOP_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Runloop({ bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Runloop({ maxRetries: 10, bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['RUNLOOP_API_KEY'] = 'My Bearer Token';
    const client = new Runloop();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['RUNLOOP_API_KEY'] = 'another My Bearer Token';
    const client = new Runloop({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('idempotency', () => {
  test('key can be set per-request', async () => {
    ...
    await client.agents.create({ name: 'name' }, { idempotencyKey: 'my-idempotency-key' });
  });
});
