...
describe('Scorer', () => {
  ...
  describe('fromId', () => {
    it('should create a Scorer instance by ID without API call', () => {
      const scorer = Scorer.fromId(mockClient, 'scr-123');

      expect(scorer).toBeInstanceOf(Scorer);
      expect(scorer.id).toBe('scr-123');
    });
  });

  describe('id property', () => {
    it('should return the scorer ID', () => {
      const scorer = Scorer.fromId(mockClient, 'scr-123');
      expect(scorer.id).toBe('scr-123');
    });
  });
  ...
  describe('getInfo', () => {
    it('should retrieve scorer information from API', async () => {
      ...
      expect(mockClient.scenarios.scorers.retrieve).toHaveBeenCalledWith('scr-123', undefined);
    ...
    it('should pass options to the API client', async () => {
      ...
      expect(mockClient.scenarios.scorers.retrieve).toHaveBeenCalledWith('scr-123', { timeout: 5000 });
    });
  });
  ...
  describe('error handling', () => {
    it('should handle API errors on getInfo', async () => {
      ...
      const scorer = Scorer.fromId(mockClient, 'scr-nonexistent');
