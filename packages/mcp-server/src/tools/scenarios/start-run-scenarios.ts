...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      benchmark_run_id: {
        type: 'string',
        description: 'Benchmark to associate the run.',
      },
      metadata: {
        type: 'object',
        description: 'User defined metadata to attach to the run for organization.',
        additionalProperties: true,
      },
      run_name: {
        type: 'string',
...
export const handler = async (client: Runloop, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.scenarios.startRun(body)));
};
