...
async function main() {
  ...
  // === Blueprint Example ===
  // Create a blueprint with a Node.js Dockerfile (using SDK)
  ...
  // === Devbox Example ===
  // Create a devbox from the blueprint (using SDK)
  ...
  const fileStorageObject = await StorageObject.uploadFromFile(
    sdk.api,
    './package.json',
  ...
  const bufferStorageObject = await StorageObject.uploadFromBuffer(
    sdk.api,
    bufferData,
    'buffer-data.bin',
    'binary',
  ...
  const blueprintFromId = await sdk.blueprint.fromId(blueprint.id);
  const devboxFromId = await sdk.devbox.fromId(devbox.id);
  const storageFromId = await sdk.storageObject.fromId(storageObject.id);
